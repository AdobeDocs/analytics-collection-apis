...
description: Use visitor groups to parallelize Bulk Data Insertion API processing and plan upload capacity.
---
...
## Example: dividing visitors into groups

Suppose a set of server calls has integer visitor IDs 1-100, and you want to create three disjoint visitor group sets. You can use the mathematical MOD operation to organize these visitors into three groups. Server calls where `visitor ID MOD 3 = 0` go into visitor group `0`. Server calls where `visitor ID MOD 3 = 1` go into visitor group `1`, and so forth. Server calls are batched into files and ordered by timestamp, per their visitor group, and are then uploaded with that visitor group specified in the header of the API request. Since the visitors in these files are all disjoint, the BDIA system can process them in parallel without risking any calls for a visit being processed out of order.

![Visitor group diagram](bia-visitor-groups.jpg)

## Naming visitor groups

For optimal ingestion efficiency, Adobe requests that your visitor group ID names incorporate a string related to your company or report suite. In the past, many companies have used simple sequential integers for their visitor group names (for example, 0, 1, 2, 3), resulting in many visitor groups of the same name across clients. Instead, append a string to your group name, such as `mycompany_1` or `myreportsuite_1`. Adobe does not enforce unique visitor group names across clients, but doing so improves the processing latency of your files.
...
When uploading a file through the REST API, it is important to understand that the files are processed in the order they are received (per visitor group). If you try to upload two files at once for the same visitor group, whichever REST call receives a "200 OK" first is processed by the system first. Because of possible race conditions, it is important to upload files within a visitor group one at a time, waiting for a "200 OK" from the server that a file has been successfully uploaded before uploading another.

## Sizing and frequency recommendations

Bulk Data Insertion was designed to run optimally with larger file sizes. Adobe recommends a pattern of larger files uploaded less frequently rather than small files uploaded more frequently.
...
File size varies according to the average size of each row. While BDIA is optimized for larger files sent less frequently, the API can only handle compressed files of up to 100 MB. However, files of this size are usually only used for historical ingest scenarios. Existing clients tend to send files with 5,000 to 50,000 rows, and sizes of 500k up to 20 MB.
